...
class ReachabilityClassifier(nn.Module):
    ...
    def get_checkpoint(self) -> Dict:
        ...
        Returns a dict containing:
          ...
          - config: the kwargs needed to reinstantiate this exact architecture
        """
        cfg = {
            "in_channels_node": self.state_gnn1.in_channels,
            "hidden_dim": self.state_gnn1.out_channels,
            "use_goal": self.use_goal,
            "num_relations": self.state_gnn1.num_relations,
        }
        ckpt = {
    ...
    def load_model(cls, ckpt: Dict):
        """
        Reconstructs a ReachabilityClassifier from a checkpoint dict
        produced by get_checkpoint().
        """
        cfg = ckpt["config"]
        model = cls(
            in_channels_node=cfg["in_channels_node"],
